...
# ======== Inisialisasi SymSpell untuk Koreksi Typo ========
sym_spell = SymSpell(max_dictionary_edit_distance=4, prefix_length=7)  # Memperbesar max_edit_distance
dictionary_path = "kbbi.txt"
sym_spell.load_dictionary(dictionary_path, term_index=0, count_index=1)

def correct_typo(text):
    # Cek SymSpell untuk koreksi
    suggestions = sym_spell.lookup(text, Verbosity.CLOSEST, max_edit_distance=4)
    if suggestions and suggestions[0].distance <= 4:  # Batasi jarak edit lebih besar
        print(f"Typo dikoreksi (SymSpell): {text} -> {suggestions[0].term}")  # Debugging log
        return suggestions[0].term
    
    # Jika SymSpell gagal, gunakan difflib untuk mencari kecocokan terbaik
    close_matches = get_close_matches(text, sym_spell.lookup("", Verbosity.ALL, max_edit_distance=4), n=1)
    if close_matches:
        print(f"Typo dikoreksi (difflib): {text} -> {close_matches[0]}")  # Debugging log
...
def correct_typo_cached(word):
    if word in typo_cache:
        return typo_cache[word]
    corrected = correct_typo(word)
    typo_cache[word] = corrected
    return corrected
...
def preprocess_text(text):
    ...
    # Koreksi typo dengan SymSpell atau difflib
    words = [correct_typo_cached(word) for word in words]  # Gunakan cache
